  ...
  cart.forEach((item) => {
    ...
    fetch(`https://api.mercadolibre.com/items/${id}`)
      ...
      .then(() => {
        ...
        console.log(item, cart[cart.length - 1])
        if (item === cart[cart.length-1]) document.querySelector('.total-price').innerText = total;
      });
    if (item === cart[cart.length-1]) finishLoading(); 
  });
  if (cart.length === 0) {
    finishLoading();
