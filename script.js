...
async function getItemsFromApi(product) {
  const apiProduct = await fetch(`https://api.mercadolibre.com/sites/MLB/search?q=${product}`);
...
async function appendListItems() {
  ...
  const response = await getItemsFromApi('computador');
